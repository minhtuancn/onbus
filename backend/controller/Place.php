...
$model = new Place;
$act = $_POST['act'];
if($act == "getPlaceByTinh"){
	$tinh_id = (int) $_POST['tinh_id'];
	$arrPlace = $model->getListPlace(-1,$tinh_id,'', -1, -1);
	...
	if($place_id > 0) {	
		$model->updatePlace($place_id,$tinh_id,$mien_id,$place_name_vi,$place_name_en,$place_name_safe_vi,$place_name_safe_en,$address_vi,$address_en);
		header('location:'.$url);
	}else{
		$model->insertPlace($tinh_id,$mien_id,$place_name_vi,$place_name_en,$place_name_safe_vi,$place_name_safe_en,$address_vi,$address_en);
		header('location:'.$url);
	}
}
